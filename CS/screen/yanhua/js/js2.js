// 当动画在画布上，最好使用requestanimationframe代替setTimeout或setInterval
		...
		var canvas = document.getElementById('canvas'), ctx = canvas.getContext('2d'),
			...
			// 存色调
			hue = 120,
			// 当发射烟花时，点击，太多的启动，没有一个限制器，每5个循环蜱一次发射
			limiterTotal = 1, limiterTick = 0,
			// 这将时间自动启动的烟花，每80个循环蜱一次发射
			timerTotal = 80, //80
			timerTick = 0,
			mousedown = false,
			// 鼠标的X坐标,
		...
		function loop() {
			...
			// 发射烟花自动到随机坐标，发射烟花			
			if( timerTick >= timerTotal ) { //timerTotal
				if( !mousedown ) {
					
					// 在屏幕的底部开始烟火，然后设置随机目标坐标，随机的Y坐标将被设置在屏幕的上半部分的范围内
